...
const RoomPage = () => {
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white border rounded-md shadow-md text-center">
      <h1 className="text-3xl font-bold mb-4">Room Page</h1>
      <h4 className="mb-4">{remoteSocketId ? "Connected" : "No one in room"}</h4>
      {myStream && (
        <button onClick={sendStreams} className="bg-blue-500 text-white px-4 py-2 rounded-md mr-2">
          Send Stream
        </button>
      )}
      {remoteSocketId && !myStream && (
        <button onClick={handleCallUser} className="bg-green-500 text-white px-4 py-2 rounded-md">
          CALL
        </button>
      )}
      {myStream && (
